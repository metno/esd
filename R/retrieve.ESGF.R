...
meta.ESGF <- function(url="https://esgf-data.dkrz.de/esg-search/search/",mip="CMIP6",param="tas",
                      freq="mon",expid="ssp585",verbose=FALSE) {
  ...
  for (i in 1:nof_datasets) {
    ...
    for (j in 1:nof_files) {
      ...
      results[[paste('member_id',ic,jc,sep='_')]] <- ESGF_file_query$response$docs[[1]]$member_id
      results[[paste('grid',ic,jc,sep='_')]] <- ESGF_file_query$response$docs[[1]]$grid
      results[[paste('source_id',ic,jc,sep='_')]] <- ESGF_file_query$response$docs[[1]]$source_id
      results[[paste('type',ic,jc,sep='_')]] <- ESGF_file_query$response$docs[[1]]$source_type
    }
  }

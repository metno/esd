...
metno.frost.station <- function(keyfile='~/.FrostAPI.key', url='https://frost.met.no/auth/requestCredentials.html',
  ...
  if (!requireNamespace("jsonlite", quietly = TRUE)) {
    ...
    ## REB 2020-11-27: there is a problem with entiries like as.POSIXlt("1895-01-01")
    ...
    if (!inherits(test.date,"try-error")) { 
      ndata <- switch(toupper(timeresolutions),
                      ...
                      "PT1M"=difftime(end.j, start.j, units="minutes")) } else
              ndata <- as.numeric(as.Date(end.j) - as.Date(start.j))/switch(toupper(timeresolutions),"P1M"=30,"P1D"=1,"PT1M"=1/(24*3600))

...
##     ## use.cdfcont = FALSE - AM 22-10-2013 not used any more ! 
...
check.ncdf2 <- function(ncid, param="auto",verbose = FALSE) { ## use.cdfcont = FALSE - AM 22-10-2013 not used any more ! 
        ...
        if (ncid$nvars > 1) {
            ...
            if (length(i) == 0)
                i <- as.integer(readline(paste("Choose variable ",
                                               paste(namevars,collapse="/") ,"(from 1 - ",
                                               length(namevars), "): ",sep = "")))
            if (!is.integer(i))
...
check.ncdf <- function(ncid, param="auto",verbose = FALSE) { ## use.cdfcont = FALSE - AM 22-10-2013 not used any more ! 
    ...
    if (!is.null(calendar.att)) {
        ...
        if (grepl("mon",tunit)) {
            if (sum(diff(time$vals>1)) < 1) {
                year1 <- time$vals[1]%/%12 + yorigin
                month1 <- morigin
                time$vdate <- seq(as.Date(paste(as.character(year1),month1,"15",sep="-")), by = "month",length.out=time$len)
            } else warnings("Monthly data are Mangeled") 
        } 
    }
    ##browser()
    if ((length(time$vdate)>0) & (sum(diff(as.numeric(format.Date(time$vdate,"%m")))>1)) & (verbose)) {
        print("Warning: Vector date is mangeled ! Need extra check !")
        print("Vector date reconstructed as a sequence based on the first date and the time length")
    ...
    if (!is.null(time$vdate)) {
        ...
        if ((length(dt)==3) & grepl("day",tunit)) {
            if (verbose)
                print(paste("Calendar is likely to be a 365-",tunit," with: ",as.character(length(dt))," irregular frequencies",sep = ""))
            dt <- c(28,30,31)
            if (verbose)

...
map.station.old <- function(x=NULL,FUN=NULL, it=NULL,is=NULL,new=FALSE,
  ...
  if (sum(is.element(type,c('fill','contour')))) {
    ...
    x0 <- x
    x <- as.field(x)
    z <- map(x,lonR=lonR,latR=latR,axiR=axiR,projection=projection,xlim=xlim,ylim=ylim,
  ...
  if(projection=="sphere") {
    ...
    projection <- "sphere"
    stereographic <- TRUE
    ## KMP 2025-10-10: The stereographic projection looks best from the poles or the equator
    if(is.null(latR)) {
      if(mean(lat(x))>45) latR <- 90 else
        if(mean(lat(x))<-45) latR <- -90 else
          latR <- 0
    }

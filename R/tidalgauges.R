...
station.gloss <- function(url='http://browse.ceda.ac.uk/browse/badc/CDs/gloss/data') {
  ...
  for (i in 1:n) {
    ii <- is.element(SL$location,names(Xt)[i])
  ...
  lon <- glossstations$V5[is.element(toupper(loc),toupper(glossstations$V1))]
  lat <- glossstations$V4[is.element(toupper(loc),toupper(glossstations$V1))]
  cntr <- glossstations$V2[is.element(toupper(loc),toupper(glossstations$V1))]
